...
| $action | \`$skill_name\` |"
    ...
    if $DRY_RUN; then
        ...
        if grep -q "### Auto-invoke Skills" "$agents_path"; then
        ...
        elif grep -q "<!-- AUTO_INVOKE -->" "$agents_path"; then
            # Replace marker with Auto-invoke table (first run from template)
            awk '
                /<!-- AUTO_INVOKE -->/ {
                    while ((getline line < "'"$section_file"'") > 0) print line
                    close("'"$section_file"'")
            ...
            ' "$agents_path" > "$agents_path.tmp"
            mv "$agents_path.tmp" "$agents_path"
            echo -e "${GREEN}  ✓ Replaced AUTO_INVOKE marker with Auto-invoke section${NC}"
        else
