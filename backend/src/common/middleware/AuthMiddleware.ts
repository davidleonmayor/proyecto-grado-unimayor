...
export class AuthMiddleware {
  ...
  public async isConfirmed(req: Request, res: Response, next: NextFunction) {
    try {
      // CASO A: Usuario ya autenticado (pasó por isAuthenticatedUser)
      if (req.user) {
        if (!req.user.confirmed) {
          logger.warn(`Acceso no confirmado: ${req.user.correo_electronico}`);
      ...
      // CASO B: Rutas públicas (ej. /login, /forgot-password) donde no hay sesión
      // pero necesitamos verificar si el dueño de ese correo está confirmado.
      const { email } = req.body;

      if (!email) {
        logger.error(
          "Intento de validar confirmación sin sesión ni email en el body.",
        );
        return res.status(400).json({
          success: false,
          message:
            "No se proporcionó un email para validar el estado de la cuenta.",
        });
      }
      const persona = await prisma.persona.findUnique({
      ...
      if (!persona.confirmed) {
        logger.warn(
          `Acceso no confirmado (ruta pública): ${persona.correo_electronico}`,
        );
        return res.status(403).json({
          success: false,
          message: "Debes confirmar tu cuenta antes de continuar",
      ...
      // Si llegó hasta acá, la cuenta existe y está confirmada.
      req.user = persona; // Solo con correo y estado confirmado, para rutas públicas
      logger.debug(`Cuenta confirmada para: ${persona.correo_electronico}`);

      next();
    } catch (error: any) {
      logger.error(`Error en isConfirmed: ${error.message}`);
      return res.status(500).json({
        success: false,
        message: "Error interno al validar confirmación de cuenta",
      });
    }
  }
}
